...
*From chaotic reports to coordinated response — in real time.*

[![Frontend](https://img.shields.io/badge/Frontend-React%20%2B%20Vite-61DAFB?logo=react&logoColor=white)](#-tech-stack)
[![Backend](https://img.shields.io/badge/Backend-Node.js%20%2B%20Express-339933?logo=node.js&logoColor=white)](#-tech-stack)
[![Realtime](https://img.shields.io/badge/Realtime-Socket.IO-010101?logo=socket.io&logoColor=white)](#-tech-stack)
[![Database](https://img.shields.io/badge/Database-PostgreSQL-4169E1?logo=postgresql&logoColor=white)](#-tech-stack)
[![Built For](https://img.shields.io/badge/Built%20for-24--Hour%20Hackathon-orange)](#-team)

</div>

---

SURGE takes dozens of chaotic reports arriving from many channels and collapses them into a handful of clear, prioritized incidents. It tells the commander **exactly which resource to send and why**, and it **re-plans instantly** the moment a resource fails. A human commander always makes the final call — SURGE reads, ranks, explains, and drafts; the commander approves.
...
During large-scale emergencies — floods, fires, industrial accidents, major road incidents — information arrives from many disconnected sources at once:
...
A control-room dispatcher has to make sense of all of it simultaneously. In practice, this causes:
...
> A human commander always makes the final call. The AI reads, ranks, explains, and drafts — the commander approves.
...
### 📥 Incident Collection
Reports arrive from citizens, emergency calls, sensors, and field teams into a single, normalized incident pipeline.

### 🧠 AI Classification & Severity
Each incident gets a type, severity score, priority (P1–P4), confidence, and people-at-risk estimate — with a structured, explainable factor breakdown.
...
### 📂 Evidence Panel
Every incident shows the evidence behind it — citizen reports, calls, sensor readings, and field updates — and how each one contributes.
...
### 🎮 Demo Controls
Buttons to generate a flood, fire, accident, duplicate reports, resource failure, and delays — so the full workflow can be demonstrated live.
...
Emergency decisions must be auditable, so we deliberately separate **what the AI reads** from **what the rules decide**.
...
- Node.js 18+
- npm
- PostgreSQL (only needed when running the real backend)
...
- [x] Mock data layer and state contexts
- [x] Incident list, details, and live map
- [x] AI command panel, explainability, and evidence
- [x] Duplicate detection and merge flow
- [x] Recommendation and dispatch approval
- [x] Dynamic response recovery
- [x] Socket.IO integration, alerts, and timeline
- [x] Analytics dashboard
- [x] Demo controls and final polish
...
| Backend & AI | Parita ([@parita-31](https://github.com/parita-31)) |
...
**SURGE — because in an emergency, every second of coordination saved is a life protected.**
